...
function ScanPay() {
  ...
  const availableCryptos = [
    {
      ...
      icon: (
        <Suspense fallback={null}>
          <BitcoinIcon size={20} className="text-yellow-500" />
        </Suspense>
      ),
      symbol: "BTC",
    },
    {
      ...
      icon: (
        <Suspense fallback={null}>
          <ArrowLeftRightIcon size={20} className="text-crypto-purple" />
        </Suspense>
      ),
      symbol: "ETH",
    },
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        console.log("Transaction submitted with:", { amount, sender });
      } else {
  ...
  return (
    <Suspense
      fallback={
        <div className="text-white text-center py-10">Loading ScanPay...</div>
      }
    >
